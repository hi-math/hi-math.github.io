...
$ f(x)=\sum_c \pi_c N(x \vert \mu_c,\sigma_c) $
...
$\omega^{(i)}_j = p(z^{(i)} = j \vert x^{(i)};𝜙,𝜇,𝛴) $ 라는 식으로 쓰여지고 파라미터로 𝜙,𝜇,𝛴 를 사용하게 됩니다.
...
$\omega^{(i)}_여 = p(z^{(i)} = 여 \vert x^{(i)}=256)$

$\omega^{(i)}_남 = p(z^{(i)} = 남 \vert x^{(i)}=256)$
...
$= \frac {p(x^{(i)}\vert z^{(i)} = j ; \mu,\sigma)p(z^{(i)} = j; \phi)}{p(x^{(i)};\phi,\mu,\Sigma)}$

$=\frac {p(x^{(i)}\vert z^{(i)} = j ; \mu,\Sigma)p(z^{(i)} = j; \phi)}{\sum^l_{k=1} p(x^{(i)}\vert z^{(i)} = k ; \mu,\Sigma) p(z^{(i)} = k ; \phi)}$
